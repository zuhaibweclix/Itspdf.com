...
/**
 * PDF to BMP Converter — QA Audit Suite
 * Built on the QA-Auditor framework: SEO QA · UX QA · Functionality QA.
 * URL: https://www.itspdf.com/pdf-to-bmp
 *
 * ───────────────────────────────────────────────────────────────────────────
...
const CONFIG = {
  ...
  path: '/pdf-to-bmp',
  toolName: 'PDF to BMP',
  title: 'PDF to BMP converter | fast & free image Conversion',
  description:
    'Pdf to bmp converter is free and fast, user no need to sign up. This online tool lets you easy conversions of pdf files into high quality bitmap images.',
  imageExt: 'bmp',
};
...
const SEL = {
  ...
  // The single text "Download" anchor in .result-actions (works for both the
  // .zip result and the single-page .bmp result — only the href differs).
  ...
  // Single-page BMP result: a hover overlay reveals this download link.
  // It is in the DOM even when hidden, so .should('exist') works without hover.
  singleFileDownload: '.single-file-download-overlay a[download]',
  // Delete button inside .result-actions — only appears when 2+ files are
  // converted. Not present for a single-file result.
...
/**
 * @param {number} gateTimeoutMs  How long to wait for the result to render.
 *   Converting many files (e.g. the 10-file max-limit test) is the heaviest
 *   operation in the suite, so callers with a large batch should pass a
 *   longer timeout instead of relying on the 60s default.
 */
function downloadAndVerifyZip(gateTimeoutMs = 60000) {
  cy.get(SEL.downloadGate, { timeout: gateTimeoutMs }).should('be.visible');
  cy.get('body').then(($b) => {
    const hrefs = [
      ...new Set(
    ...
    hrefs.forEach((href) => {
      // The server can still be busy finishing a large ZIP right after a
      // heavy multi-file conversion (e.g. the 10-file test), so this request
      // needs its own generous timeout — the global responseTimeout (30s) is
      // too tight here and was causing cy.request to time out, which then
      // cascaded into the next test failing to load the page at all.
      // failOnStatusCode:false so a slow/erroring asset can't throw uncaught.
      cy.request({
        url: href,
...
describe(`${CONFIG.toolName} — QA Audit Suite (SEO · UX · Functionality)`, () => {
  beforeEach(() => {
    ...
    // failOnStatusCode:false + a generous pageLoadTimeout-aligned wait so a
    // server that's still busy right after the previous (heavy) test doesn't
    // cause this visit to throw and skip the rest of the suite.
    cy.visit(CONFIG.url, { failOnStatusCode: false, timeout: 90000 });
  });
  ...
  context('SEO QA', () => {
    ...
    it('has exactly one H1 containing the primary keyword', () => {
      ...
      cy.get('h1').invoke('text').then((t) => {
        expect(t.toLowerCase()).to.match(/pdf to (bmp|bitmap)/);
      });
    });
  ...
  context('Functionality QA — File management controls', () => {
    ...
    it('blocks the 11th file, then converts & downloads the valid 10', () => {
      ...
      clickConvert();
      // Converting 10 files to BMP is the heaviest operation in this suite —
      // give it a generous 180s instead of the 60s default so a slow server
      // response fails this one test cleanly rather than throwing uncaught
      // and cascading into skipped tests further down the file.
      downloadAndVerifyZip(180000);
      // Brief settle so the server isn't immediately hit again by the next
      // test's beforeEach visit while it's still finishing up this ZIP.
      cy.wait(3000);
    });
  });
  ...
  context('Functionality QA — Convert, download, reset', () => {
    ...
    it('offers a direct .bmp download for a single-page PDF', () => {
      // A single-page PDF produces a .bmp file — the download link appears
      // inside a hover overlay (.single-file-download-overlay). No delete
      // button appears for a single converted file. Requires a 1-page fixture.
      chooseFiles('single-page.pdf');
      waitForUploads(1);
      clickConvert();
      cy.get(SEL.downloadGate, { timeout: 60000 }).should('be.visible');

      // Hover the converted file thumbnail to reveal the overlay.
      cy.get('.otherfile, .file-card, [class*="result"]').first()
        .trigger('mouseover', { force: true });

      // Verify the overlay link exists and carries a .bmp href — no click,
      // no cy.request (avoids hammering the server after conversion).
      cy.get(SEL.singleFileDownload, { timeout: 15000 }).should('exist');
      cy.get(SEL.singleFileDownload).first().then(($a) => {
        const href = $a.attr('href');
        if (!href || /\.zip(\?|$)/i.test(href)) {
          finding('single-page PDF produced a direct .bmp (not zip)', false, 'fixture may not be 1 page');
          return;
        }
        expect(href, 'single-file href').to.match(new RegExp(`\\.${CONFIG.imageExt}(\\?|$)`, 'i'));
      });
    });
  ...
  context('Responsive QA — Devices & Breakpoints', () => {
    ...
    // Each test sets cy.viewport BEFORE its own cy.visit, and is fully
    // independent — if one viewport's page load is slow/fails, Mocha treats
    // it as one failing test rather than aborting the remaining iterations
    // in this forEach (which previously caused a cascade of skipped tests).
    VIEWPORTS.forEach(({ label, w, h }) => {
      it(`[${label} ${w}×${h}] — no horizontal overflow, upload reachable`, () => {
        ...
        cy.get(SEL.firstFileInput, { timeout: 15000 }).should('exist');

        // Choose Files button is visible and has a tappable height
        cy.get(SEL.chooseFilesBtn, { timeout: 15000 }).should('be.visible').then(($b) => {
