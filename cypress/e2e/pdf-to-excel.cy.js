...
/**
 * PDF to Excel Converter — QA Audit Suite
 * Built on the QA-Auditor framework: SEO QA · UX QA · Functionality QA.
 * URL: https://www.itspdf.com/pdf-to-excel
 *
 * ───────────────────────────────────────────────────────────────────────────
 * OUTPUT TYPE  : xlsx (single-file tool — no per-file result rows)
 * UPLOAD LIMIT : 1 file(s) at once
 *
 ...
 *   (no multi-file fixtures needed — single file only)
 *
...
const CONFIG = {
  ...
  title:      'PDF to Excel Converter: Convert PDF Tables to Excel',
  description:'Convert your PDF file into an editable Excel spreadsheet in just a few clicks. Accurately export PDF to Excel so you can analyze and update Excel files.',
  outputExt:  'xlsx',
  fileLimit:  1,
};
  ...
  cy.get('body').then(($b) => {
    ...
    hrefs.forEach((href) => {
      cy.request(href).then((resp) => {
        ...
        expect(
          /xlsx|spreadsheetml|openxmlformats|ms-excel|octet-stream/.test(type) || new RegExp(`\\.${ext}`, 'i').test(href),
...
describe(`${CONFIG.toolName} — QA Audit Suite (SEO · UX · Functionality)`, () => {
  ...
  context('SEO QA', () => {
    ...
    it('has exactly one H1 containing the primary keyword', () => {
      ...
      cy.get('h1').invoke('text').then((t) => {
        expect(t.toLowerCase()).to.match(/pdf to excel/);
      });
    });
  ...
  context('Functionality QA — File management controls', () => {
    it('shows the "+" affordance is absent (single-file tool)', () => {
      // pdf-to-excel accepts only 1 file; the add-more affordance should not
      // be active after a file is already uploaded.
      chooseFiles('valid.pdf');
      waitForUploads(1);
  ...
  context('Functionality QA — Convert, download & result controls', () => {
    ...
    it('accepts only 1 file (upload limit is 1)', () => {
      // pdf-to-excel is a single-file-only tool.
      chooseFiles('valid.pdf');
      waitForUploads(1);
      expectListed('valid');
      cy.get(SEL.toast).should('not.exist');
