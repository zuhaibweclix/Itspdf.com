...
/**
 * PDF to TIFF Converter — QA Audit Suite
 * Built on the QA-Auditor framework: SEO QA · UX QA · Functionality QA.
 * URL: https://www.itspdf.com/pdf-to-tiff
 *
 * ───────────────────────────────────────────────────────────────────────────
...
const CONFIG = {
  url: 'https://www.itspdf.com/pdf-to-tiff',
  path: '/pdf-to-tiff',
  toolName: 'PDF to TIFF',
  title: 'PDF to TIFF converter - fast and easy conversion',
  description:
    'This online pdf to tiff converter lets you turn your pdf files to multipage TIFF images without need to sign up or any software installation.',
  imageExt: 'tiff',
};
...
function downloadAndVerifyZip() {
  // Only verify the result area is visible — no URL fetching so the server
  // is not hammered right after a heavy conversion.
...
describe(`${CONFIG.toolName} — QA Audit Suite (SEO · UX · Functionality)`, () => {
  ...
  context('SEO QA', () => {
    ...
    it('has exactly one H1 containing the primary keyword', () => {
      ...
      cy.get('h1').invoke('text').then((t) => {
        expect(t.toLowerCase()).to.match(/pdf to (tiff|tif)/);
      });
    });
  ...
  context('Functionality QA — Convert, download, reset', () => {
    ...
    it('offers a direct .tiff download for a single-page PDF', () => {
      // A single-page PDF produces a direct .tiff file (visible in the
      // result list); multi-page PDFs produce a .zip. The download link
      // appears inside a hover overlay. Requires a true 1-page fixture.
      ...
      // The overlay anchor is in the DOM even when hidden.
      ...
      cy.get(SEL.singleFileDownload).first().then(($a) => {
        ...
        if (!href || /\.zip(\?|$)/i.test(href)) {
          finding('single-page PDF produced a direct .tiff (not zip)', false, 'fixture may not be 1 page');
          return;
        }
        expect(href, 'single-file href').to.match(new RegExp(`\\.${CONFIG.imageExt}(\\?|$)`, 'i'));
        // Verify href format only — no URL fetch to avoid server load.
        finding('single-page .tiff href is present', true, href);
      });
    });
