...
/**
 * PDF to PowerPoint Converter — QA Audit Suite
 * Built on the QA-Auditor framework: SEO QA · UX QA · Functionality QA.
 * URL: https://www.itspdf.com/pdf-to-powerpoint
 *
 * ───────────────────────────────────────────────────────────────────────────
 * OUTPUT TYPE  : pptx (per-file result rows appear for 2+ files)
...
const CONFIG = {
  ...
  title:      'Convert PDF to PPT - online PDF to Powerpoint converter',
  description:'The online pdf to powerpoint converter compatible for all devices and browsers. The online tool is free, fast, and easy to use.',
  outputExt:  'pptx',
  fileLimit:  10,
};
...
function downloadAndVerifyOutput() {
  ...
  cy.get('body').then(($b) => {
    const ext = CONFIG.outputExt;
    const hrefs = [
      ...new Set(
        [...$b.find('.result-actions a[download][href]')]
          .map((a) => a.getAttribute('href'))
          .filter((h) => new RegExp(`\\.${ext}(\\?|$)`, 'i').test(h))
      ),
    ];
    hrefs.forEach((href) => {
      cy.request(href).then((resp) => {
        ...
        expect(
          /pptx|presentationml|openxmlformats|octet-stream/.test(type) || new RegExp(`\\.${ext}`, 'i').test(href),
          `served a ${ext} (content-type="${type}")`
        ).to.be.true;
      });
    });
  });
...
describe(`${CONFIG.toolName} — QA Audit Suite (SEO · UX · Functionality)`, () => {
  ...
  context('SEO QA', () => {
    ...
    it('has exactly one H1 containing the primary keyword', () => {
      ...
      cy.get('h1').invoke('text').then((t) => {
        expect(t.toLowerCase()).to.match(/pdf to (ppt|powerpoint)/);
      });
    });
  ...
  context('Functionality QA — Convert, download & result controls', () => {
    it('converts a single PDF and shows the #downloadAll button', () => {
      ...
      waitForUploads(1);
      clickConvert();
      cy.get(SEL.downloadAll, { timeout: 90000 }).should('be.visible');
    });
    ...
    it('#downloadAll button label reads "Download" after conversion', () => {
      ...
      waitForUploads(1);
      clickConvert();
      cy.get(SEL.downloadAll, { timeout: 90000 }).should('be.visible').and('contain.text', 'Download');
    });
    ...
    it('converts multiple PDFs and shows a per-file .result-actions row each', () => {
      ...
      waitForUploads(2);
      clickConvert();
      cy.get(SEL.downloadLink, { timeout: 90000 }).should('have.length.gte', 2);
    });

    it('per-file .pptx links carry the correct file extension', () => {
      chooseFiles(['valid.pdf', 'valid-2.pdf']);
      waitForUploads(2);
      clickConvert();
      cy.get(SEL.downloadLink, { timeout: 90000 }).should('be.visible');
      cy.get(SEL.downloadLink).each(($a) => {
        cy.wrap($a).should('have.attr', 'href').and('match', /\.pptx(\?|$)/i);
      });
    });

    it('per-file .pptx link serves a real output file', () => {
      chooseFiles(['valid.pdf', 'valid-2.pdf']);
      waitForUploads(2);
      clickConvert();
      cy.get(SEL.downloadLink, { timeout: 90000 }).should('be.visible');
    ...
    it('result-area Delete button removes a converted file (multi-file only)', () => {
      ...
      waitForUploads(2);
      clickConvert();
      cy.get(SEL.resultDelete, { timeout: 90000 }).should('have.length.gte', 2);
    ...
    it('downloads and verifies output files via all download controls', () => {
      chooseFiles(['valid.pdf', 'valid-2.pdf']);
      waitForUploads(2);
      clickConvert();
